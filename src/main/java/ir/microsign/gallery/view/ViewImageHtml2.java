 ...
 */
public class ViewImageHtml2 extends ViewHtml {
    ...
    public ViewImageHtml2(Context context) {
        super(context);
    }

    public ViewImageHtml2(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
    }

    public ViewImageHtml2(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    String prepareContent(String src, String splitter, boolean showBeforeImage, boolean showAfterImages, boolean showFirstImage) {
        ...
        for (int blockIndex = 0, arrLength = arr.length; blockIndex < arrLength; blockIndex++) {
            ...
            for (int i = 0; i < matches.size(); i++) {
                ...
                final Image img = new Image(match);
                ...
                if (!img.Exist()) {
                    img.download(new DownloadSilentFile.DownloadCompleted(){
                        @Override
                        public void OnDownloadCompleted(boolean succeed, String path) {
                            ...
                            ViewImageHtml2.this.setContent();
                        }
                    });
                }
            }
...
//					}
            ...
            Log.e("ViewImageHtml", e.getMessage() + ":" + uri);
            e.printStackTrace();
            if (mOnUrlClickListener != null)return mOnUrlClickListener.onClicked(this, url);
            return false;
        }
