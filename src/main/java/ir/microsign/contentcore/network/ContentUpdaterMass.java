 ...
 */
public class ContentUpdaterMass {
...
//    public int counter = 0;
    ...
    public ContentUpdaterMass(Activity activity, ContentUpdateAllListener l) {
...
//                if (success) updateGetOfflines(params);
//                else l.OnFinished(false);
//            }
//        });

    }
    public void updateGetOfflines(final Map<String,String> params) {
        if (checkCancel()) return;
        l.OnLastUpdateReceiveStart();
...
public void prepareToStartDownload(Update update){
    ...
    startReceiving(update);
}
void startReceiving(final Update update){
if (update==null){l.OnFinished(false);return;}
        l.OnDownloadBegin(update.zipsize);
...
//        f.GetFile(1024*5, File.GetRoot(activity,true)+"/temp",update.getUrl(),l1);
        f.downloadFile(update.getUrl(),File.GetRoot(activity,true)+"/temp",1024*5, l1);
    }
   public Update getLastExistUpdate(){
    ...
    void  unzip(final Update update,final String zipFilePath){
        l.OnUnzipStarted();
        ...
        zip.Unzip(zipFilePath,update.getExtractPath(), 1024 * 5, new Zip.OnExtractListener() {
            ...
            public boolean extracting(int extracted) {
                 l.OnUnziping(update.dbsize,extracted);
                return(!checkCancel());
            }
    ...
    public void restoreTemps(Update update){
        l.OnAfterUnzip();
        List<Temp> lst= (List<Temp>) DataSourceTemp.getDataSource(activity).select(new Temp());
        DataSource dstemp=Helper.getHelper().getDataSource();
        for (Temp o:lst)
    ...
    public interface ContentUpdateAllListener {
        ...
        public boolean OnLastUpdateReceiveEnd(boolean succeed, Update update);
