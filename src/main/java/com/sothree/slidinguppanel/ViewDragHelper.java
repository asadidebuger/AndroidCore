    ...
    public boolean continueSettling(boolean deferCallbacks) {
        ...
        if (mDragState == STATE_SETTLING) {
            ...
            if (!keepGoing && dy != 0) { //fix #525
                //Invalid drag state
                final int left = 0;
                final int top = 0;// mCapturedView.getHeight() - mCapturedView.getHeight();
                final int right = left + mCapturedView.getWidth();
                final int bottom = top + mCapturedView.getHeight();
                mCapturedView.layout(left, top, right, bottom);
